...
class NLR_Requestor : public neverlosesdk::network::Requestor
{
    ...
    {
        NlLog("MakeRequest(%s, 0x%X, 0x%X)", route.data(), _, __);
        new (&out) std::string("");
        ...
        if (wroute)
        {
            ...
            if (hRequest && WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, 0))
            {
                if (WinHttpReceiveResponse(hRequest, NULL))
                {
                    ...
                    NlLog("MakeRequest: WinHttpReceiveResponse failed for %s", route.data());
            }
            else
                NlLog("MakeRequest: WinHttpSendRequest failed for %s (error=%lu)", route.data(), GetLastError());
            WinHttpCloseHandle(hRequest);
        }
        else
            NlLog("MakeRequest: malloc failed for %s", route.data());
        NlLog("MakeRequest: %s returned %zu bytes", route.data(), out.size());
    };
    void GetSerial(std::string& out, nlohmann::json& request) override
    {
        new (&out) std::string("");
    };
    void fn2() override { NtSuspendProcess(NtCurrentProcess()); };
    void fn3() override { NtSuspendProcess(NtCurrentProcess()); };
