...
pub fn parse(data: &[u8]) -> Result<ClientMsg> {
    ...
    tracing::info!(
        "client_msg type={} payload_hex={}",
        outer.msg_type,
        crate::utils::hex_dump(outer.payload, 128)
    );
    match outer.msg_type {
        ...
        6 => {
            ...
            Ok(ClientMsg::ShareClick { entry_type })
        }
        3 => {
            let inner = Table::parse(outer.payload)?;
        ...
        2 => {
            ...
            Ok(ClientMsg::DeleteEntry {
                ...
                entry_type,
                action,
                raw_payload: outer.payload.to_vec(),
            })
        }
        11 => {
            let inner = Table::parse(outer.payload)?;
...
/// Dump all readable fields from a DeleteEntry inner payload for debugging.
/// Returns a string like "f0=6 f1=1 f2=1 f3=0" showing field_id=value.
pub fn inspect_delete_payload(payload: &[u8]) -> String {
    let Ok(tbl) = Table::parse(payload) else {
        return "parse_err".to_string();
    };
    let mut parts = Vec::new();
    for field_id in 0..10 {
        if let Some(foff) = tbl.field_offset(field_id) {
            let pos = tbl.root_pos + foff;
            if pos + 4 <= tbl.buf.len() {
                let val = read_u32(tbl.buf, pos);
                parts.push(format!("f{field_id}={val}"));
            } else if pos + 1 <= tbl.buf.len() {
                let val = tbl.buf[pos];
                parts.push(format!("f{field_id}=u8:{val}"));
            }
        }
    }
    if parts.is_empty() {
